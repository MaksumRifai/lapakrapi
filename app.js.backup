...
const pedagang = [
  ...
  { id: 'POT001', nama: 'Es Potong Rasa Memori', lapak: 'A10' } // <- Data baru berhasil ditambahkan
];
...
function ok(txt) {
  ...
  hasil.innerHTML = `
    ...
    <br>
    <button onclick="simpan('${p.id}', '${p.nama}')">Simpan</button>
  `;
}
...
function renderReport() {
  let arr = JSON.parse(localStorage.getItem('lapakrapi') || '[]');
  tbl.innerHTML = '<tr><th>Waktu</th><th>Pedagang</th><th>Jenis</th></tr>';
  
  arr.forEach(r => {
    tbl.innerHTML += `<tr><td>${r.waktu}</td><td>${r.nama}</td><td>${r.jenis}</td></tr>`;
  });
}
